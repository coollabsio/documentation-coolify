...
export const ErrorCode = forwardRef<HTMLDivElement, ErrorCodeProps>(
  ({ className, children, title, code, href, ...props }, ref) => {
    ...
    const cardContent = (
      <div
        className={cn(
          'flex flex-row items-center gap-1 rounded-lg bg-fd-card p-3 text-sm text-fd-card-foreground shadow-md transition hover:shadow-lg hover:bg-fd-muted cursor-pointer overflow-hidden',
          className,
        )}
        {...props}
      >
