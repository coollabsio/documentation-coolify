<script setup lang="ts">
import { computed } from 'vue'
const props = defineProps<{
    title?: string
    image?: string
    imageAlt?: string
    imageSize?: 'small' | 'medium' | 'large'
    link?: string
}>()

const imageSizeClass = computed(() => ({
    small: 'w-[50px] h-[50px]',
    medium: 'w-[100px] h-[100px]',
    large: 'w-[150px] h-[150px]'
}[props.imageSize ?? 'medium']))
</script>

<template>
  <div class="p-1 bg-gray-50 rounded-lg dark:bg-gray-800 border border-gray-50 dark:border-gray-800 hover:border-gray-200 dark:hover:border-purple-700">
    <div class="flex flex-col h-[180px] dark:bg-gray-800 rounded-lg cursor-pointer transition-transform duration-200">
      <div class="flex-[2] bg-gradient-to-r from-[#00D1EE] via-[#08ABDD] via-[#15ACF1] via-[#1575C1] to-[#14336d] rounded-lg flex items-center justify-center">
        <img 
          :src="image" 
          :alt="imageAlt" 
          :class="[
            imageSizeClass, 
            'object-contain',
            'p-1 rounded-lg text-white',
            'bg-[#18222EDF]/70 shadow-[0px_0px_82.7px_0px_#00000091]'
          ]"
        />
      </div>
      <div class="flex items-center justify-between px-4">
        <div class="flex items-center gap-2">
          <p class="text-base font-medium m-0 text-gray-800 dark:text-gray-100">{{ title }}</p>
          <div class="w-4 h-4 text-gray-500 dark:text-gray-400">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M5 12h14M12 5l7 7-7 7" />
            </svg>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>


